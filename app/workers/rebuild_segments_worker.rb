class RebuildSegmentsWorker < SegmentsWorker
  def perform(grouping_id)
    @grouping = Grouping.find(grouping_id)
    @current_segment_number = 0
    @previous_segments_count = @grouping.segments.roots.count
    @grouping.segments.roots.map do |segment|
      segment.destroy
      @current_segment_number +=1
      at index_to_percent(@grouping.segments_count + @previous_segments_count, @current_segment_number)
    end
